...
This template deploys [Aegra](https://github.com/aegra/aegra) `0.9.24` as an authenticated Agent Protocol backend with private pgvector/PostgreSQL and Redis services. The current template release is `v1.0.0`.
...
The template supplies database and Redis references, production settings, worker concurrency, public URL, and ports. Copy `AEGRA_API_KEY` from the Aegra service variables and send it as `Authorization: Bearer …`; `/health`, `/live`, and `/ready` intentionally remain available to Railway without authentication.
