...
The current template release is `v1.0.1`. The private authoring source is `aegra-agent-backend` in this repository; Railway builds the public `tech-progress/railway-template-aegra` mirror at `release-v1`. Run `scripts/check-aegra-standalone.sh` before each release.
...
Run `./scripts/audit-template.sh TEMPLATE_ID PUBLISHED`, delete all disposable Railway projects and local Compose volumes, update findings and progress, tag this repository as `aegra-agent-backend-v1.0.1`, and tag the public mirror as `v1.0.1` with `release-v1` at the same commit.
